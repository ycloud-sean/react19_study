...
## 学习方式说明 (Learning Method)

当用户说"我准备学习 day N 的内容"时，需要按以下方式进行教学：

1. **最小粒度阅读**：将当天的学习内容拆分为最小粒度（如：版权声明、导入语句、单个函数、单个变量定义等）
2. **逐步输出**：每次只输出一个粒度的代码和讲解
3. **等待确认**：等用户说"继续"时，才输出下一个粒度
4. **方便提问**：这种方式方便用户在任何粒度提出问题，深入理解每一行代码

**示例指令：**
```
我准备学习 day 8 的内容，需要带我按最小粒度进行代码阅读，每次只输出一个粒度，待我说继续时才输出第二个粒度，这样方便我提问问题
```
